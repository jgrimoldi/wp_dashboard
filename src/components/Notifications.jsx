...
const ValuedStock = ({ token, handleErrors }) => {
  ...
  return (
    <div className='relative rounded-3xl p-5 mt-4 shadow-md bg-gradient-to-br from-[#161ECF] to-[#571CDD]'>
      <p className='flex items-start gap-2 font-bold text-base p-1 text-gray-300'>
        <span className='text-xl'><BsBagCheck /></span>
        Stock valorizado :
      </p>
      <p className='text-white text-4xl font-bold text-center tracking-tight font-[monospace]'>
        ${stockPrice}
      </p>
    </div>
  )
        ...
        <div className='flex items-center flex-wrap gap-3'>
          ...
          <div className='flex gap-4'>
            <TooltipComponent content='Filtrar informativos' position='TopCenter'>
              <button type='button' onClick={handleFilter} value='INFO' style={{ backgroundColor: '#224C24' }} className='flex items-center justify-center w-10 h-10 text-xl text-white font-bold rounded-full p-2'>{filterBy.INFO.length}</button>
            </TooltipComponent>
            ...
            <TooltipComponent content='Filtrar críticos' position='TopCenter'>
              <button type='button' onClick={handleFilter} value='CRITICAL' style={{ backgroundColor: '#B00020' }} className='flex items-center justify-center w-10 h-10 text-xl text-white font-bold rounded-full p-2'>{filterBy.CRITICAL.length}</button>
            </TooltipComponent>
          </div>
        </div>
